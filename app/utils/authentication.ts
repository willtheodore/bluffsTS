 ...
 * @returns {APIReturn<null>} a Promise containing a success message
 */
export const signInUser = async (email: string, password: string): APIReturn<null> => {
  try {
 ...
 * @returns {APIReturn<null>} a Promise containing a success message.
 */
export const createAccount = async (email: string, password: string): APIReturn<null> => {
  try {
 ...
 * @returns {APIReturn<null>} a Promise containing a success message
 */
export const addName = async (firstName: string, lastName: string): APIReturn<null> => {
  try {
...
export function validateEmail(email: string): boolean {
  try {
    ...
    if (breakIndex === -1) { throw Error("Invalid email") }

    // check for valid recipient name
    const name = eVal.slice(0, breakIndex)
    if (name.length < 1) { throw Error("Invalid email") }
    if (!checkCharacters(expandedChars, name)) { throw Error("Invalid email") }

    // check for . in suffix
    const suffix = eVal.slice(breakIndex + 1)
    const dotIndex = suffix.indexOf(".")
    if (suffix.length < 1) { throw Error("Invalid email") }
    if (dotIndex === -1) { throw Error("Invalid email") }

    // check for valid domain (pre extension)
    const domain = suffix.slice(0, dotIndex)
    if (domain.length < 1) { throw Error("Invalid email") }
    if (!checkCharacters(narrowChars, domain)) { throw Error("Invalid email") }
    ...
    if (match === undefined) { throw Error("Invalid email") }
    return true
  } catch (e) {
    throw e
  }
...
export function validatePassword(password: string) {
  try {
    ...
    if (!checkCharacters(expandedChars, password)) { throw Error("Invalid password") }

    // check for proper length
    if (length < 8 || length > 20) { throw Error("Invalid password") }
...
export function confirmNotEmpty(array: string[]) {
    for (let i = 0; i < array.length; i++) {
      if (array[i].length < 1 || array[i] === null || array[i] === undefined) { return false }
    }
    return true
}