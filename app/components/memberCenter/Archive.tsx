...
export default function Archive() {
  ...
  const getPostsForArchive = async () => {
    try {
      ...
      } else { throw Error("No value for the month and/or year. Select a value.") }
    } catch (e) {
      setError(e.message)
    }
  }
  ...
  if (error) {
    return (
      <div id="archive" className="content-wrapper">
          ...
          {posts[`${month}/${year}`].map(post => (
            <li key={post.postId} >
              <BlogPost
                ...
                date={post.formattedDate ? post.formattedDate : "MM/HH/YYYY at HH:MM AM"}
