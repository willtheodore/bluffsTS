...
export default function Comments({ comments, postId }: CommentsProps) {
	...
	const handleSubmit = () => {
		if (isNull(newComment.current) || isNull(user)) {
			return;
		}
		...
		<div style={styles.comments}>
			...
			{result === null || result === "success" ? (
				<div style={styles.wrapper}>
					<label htmlFor="addComment" style={styles.label}>
						Add a comment:
					</label>
			...
			{comments != undefined && user && <ul>{getCommentsContent(comments)}</ul>}
		</div>
	);
}
	...
	return (
		<div className="content-wrapper" style={styles.container as CSSProperties}>
			<div style={styles.descriptionWrapper as CSSProperties}>
				<p
					style={styles.description as CSSProperties}
				>{`posted by ${formattedComment.authorName} on ${formattedComment.formattedDate}`}</p>
				{uid === comment.author &&
					(!confirmDelete ? (
						<FaTrash
